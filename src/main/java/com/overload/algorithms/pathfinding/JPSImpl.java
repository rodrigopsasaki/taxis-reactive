...
public class JPSImpl implements AlgorithmDefinition {
  ...
  public List<Locatable> findPath(final Locatable start, final Locatable end) {
    ...
    try {
      ...
      if (walkable(dest, null)) { // ensure the destination isn't blocked already
        ...
        do {
          if (curr.equals(dest)) { return resolve(curr); }
          closed.add(curr);
          for (final JPNode neighbor : curr.getNeighbors(true)) {
            JPNode jumpPoint = jump(neighbor, curr);
            if (jumpPoint != null) {
              if (closed.contains(jumpPoint)) { continue; }
              if (!open.contains(jumpPoint)) {
                jumpPoint.setParent(curr);
                open.offer(jumpPoint);
              } else if ((curr.getG() + curr.getMoveCost(jumpPoint)) < jumpPoint.getG()) { // G score of node with current node as it's
                // parent
                final JPNode instanceNode = retrieveInstance(open, jumpPoint);
                if (instanceNode != null) { instanceNode.setParent(curr); }
              }
            }
          }
  ...
  private JPNode jump(Node node, Node parent) {
    ...
    if ((dx & dy) != 0) { // diagonal
      ...
      if (h != null) { return new JPNode(node); }
      JPNode v = jump(node.derive(0, dy), node);
      if (v != null) { return new JPNode(node); }
    ...
    protected JPNode[] getNeighbors(boolean eight) {
      ...
      if (parent != null) { // determines whether to prune neighbors
        ...
        if ((dx & dy) != 0) { // diagonal direction
          ...
          if (walkable(temp.set(x, y + dy), this)) { nodes.add(new JPNode(temp)); }
          if (walkable(temp.set(x + dx, y), this)) { nodes.add(new JPNode(temp)); }
          if (walkable(temp.set(x + dx, y + dy), this)) { nodes.add(new JPNode(temp)); }
          // forced neighbor checks
          if (!walkable(temp.set(x - dx, y), this)) { nodes.add(new JPNode(temp.shift(0, dy))); }
          if (!walkable(temp.set(x, y - dy), this)) { nodes.add(new JPNode(temp.shift(dx, 0))); }
        } else { // straight direction
          if (walkable(temp.set(x + dx, y + dy), this)) {
            nodes.add(new JPNode(temp));
            // forced neighbor checks
            if (!walkable(temp.set(x + dy, y + dx), this)) { nodes.add(new JPNode(temp.shift(dx, dy))); }
            if (!walkable(temp.set(x - dy, y - dx), this)) { nodes.add(new JPNode(temp.shift(dx, dy))); }
          }
        }
      } else {
        ...
        for (int i = 0; i < ns.length; i++) {
          if (walkable(ns[i], this)) { nodes.add(ns[i]); }
        }
      }
